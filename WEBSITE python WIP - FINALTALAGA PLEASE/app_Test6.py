...
parking_lot_entrance = view.importCarParkEntrance()
parking_lot_labels = view.importCarParkLotLabels()
...
def gen_frames():
    ...
    while True:
        ...
        if success:
            if (userView):
                view.mainCamera(parking_lot_coords, parking_lot_startLoc,parking_lot_entrance, parking_lot_labels,frame, 1)
            if (adminView):
                view.mainCamera(parking_lot_coords, parking_lot_startLoc,parking_lot_entrance, parking_lot_labels,frame, 2)
...
@app.route("/S4_sw.js")
def sw():
  response = make_response(send_from_directory(app.static_folder, "S4_sw.js"))
  return response
...
def push():
  ...
  message = request.form.get("adminMessage")
  title = request.form.get("adminMessageTitle")
  ...
  try:
    webpush(
      ...
      data = json.dumps({
        "title" : title,
        "body" : message,
        "icon" : "static/notif_icon.png",
        "image" : "static/notif_banner.png"
      }),
...
def adminLogin():
    if request.method == 'POST':
        ...
        query = "SELECT admin_username,admin_password FROM admin_Table WHERE admin_username ='" + username + "' and admin_password ='" + password +"'"
